...
func TestRealIP_TrustProxy_True_XForwardedFor(t *testing.T) {
	var gotAddr string
	inner := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotAddr = r.RemoteAddr
		w.WriteHeader(http.StatusOK)
	})
	handler := httpapi.RealIP(true)(inner)
	req := httptest.NewRequest(http.MethodGet, "/", nil)
	...
	if gotAddr != "203.0.113.50" {
		t.Errorf("RealIP with trust=true: got %q, want %q", gotAddr, "203.0.113.50")
	}
}

func TestRealIP_TrustProxy_True_XRealIP(t *testing.T) {
	var gotAddr string
	inner := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotAddr = r.RemoteAddr
		w.WriteHeader(http.StatusOK)
	...
	w := httptest.NewRecorder()
	handler.ServeHTTP(w, req)
	if gotAddr != "198.51.100.1" {
		t.Errorf("RealIP with trust=true X-Real-Ip: got %q, want %q", gotAddr, "198.51.100.1")
	}
}

func TestRealIP_TrustProxy_False_IgnoresHeaders(t *testing.T) {
	var gotAddr string
	inner := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotAddr = r.RemoteAddr
		w.WriteHeader(http.StatusOK)
	})
	handler := httpapi.RealIP(false)(inner)
	req := httptest.NewRequest(http.MethodGet, "/", nil)
	req.RemoteAddr = "172.17.0.1:12345"
	...
	w := httptest.NewRecorder()
	handler.ServeHTTP(w, req)
	if gotAddr != "172.17.0.1:12345" {
		t.Errorf("RealIP with trust=false should ignore proxy headers: got %q, want %q", gotAddr, "172.17.0.1:12345")
	}
}
