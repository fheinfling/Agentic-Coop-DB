...
def connect(base_url: str, *, api_key: str, timeout: float = 30.0, verify_tls: bool = True) -> AgentCoopDBClient:
    """Create a client bound to base_url + api_key.

    >>> db = connect("https://db.example.com", api_key="acd_live_...")
    """
    return AgentCoopDBClient(base_url=base_url, api_key=api_key, timeout=timeout, verify_tls=verify_tls)


class AgentCoopDBClient:
    ...
    def execute(
        ...
        *,
        ...
        # Only auto-generate an idempotency key for writes. Read-only
        # statements are inherently idempotent and don't need server-side
        # dedup; skipping avoids unbounded growth of the idempotency table.
        # WITH is excluded because CTEs can contain writes.
        is_read = sql.lstrip().upper().startswith(("SELECT", "SHOW", "EXPLAIN"))
        data = self._post(
            "/v1/sql/execute", body, headers,
            auto_idempotency=not is_read,
        )
        return ExecuteResult(
            ...
            replayed=bool(data.get("_replayed")),
        )
    ...
    def _post(
        ...
        *,
        ...
        # Auto-generate an Idempotency-Key so that transport-level retries
        # (NetworkError / ServerError) cannot cause duplicate writes. The key
        # is generated ONCE before the retry loop so every attempt uses the
        # same value.  Callers that know the request is read-only (e.g.
        # SELECT) pass auto_idempotency=False to avoid filling the server's
        # idempotency_keys table with one-shot UUIDs.
        merged: dict[str, str] = {}
        if headers:
            merged.update(headers)
        if auto_idempotency and "Idempotency-Key" not in merged:
    ...
    def _handle(r: requests.Response) -> dict[str, Any]:
        if r.status_code < 300:
            ...
            if r.headers.get("X-Idempotent-Replayed") == "true":
        ...
        raise AgentCoopDBError(detail, status=r.status_code, problem=problem)
...
class TransactionBuilder:
    ...
    def commit(self, client: AgentCoopDBClient) -> None:
