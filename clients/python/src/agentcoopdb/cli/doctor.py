...
Checks (in order):
  ...
  6. Idempotency replay probe — same key returns cached response
...
def doctor() -> None:
    ...
    try:
        rz = db._get("/readyz")
        _ok("/readyz", str(rz.get("status", "?")))
    except AgentCoopDBError as e:
        _fail(f"/readyz failed (DB or migrations unhealthy): {e}")
    ...
    # Idempotency replay probe — uses a temp uuid as the key and verifies
    # the server's X-Idempotent-Replayed header to confirm a cached replay.
    key = str(uuid.uuid4())
    try:
        db.execute("SELECT pg_sleep(0)", idempotency_key=key)
        r2 = db.execute("SELECT pg_sleep(0)", idempotency_key=key)
        if r2.replayed:
            _ok("idempotency", "replay returned cached response (X-Idempotent-Replayed confirmed)")
        else:
            _fail("idempotency replay succeeded but X-Idempotent-Replayed header missing — caching may be broken")
    except AgentCoopDBError as e:
