...
func TestMapError_ResponseTooLarge(t *testing.T) {
	p := httpapi.MapError(sqlpkg.ErrResponseTooLarge)
	if p.Status != http.StatusRequestEntityTooLarge {
		t.Errorf("status: got %d, want %d", p.Status, http.StatusRequestEntityTooLarge)
	}
	if p.Title != "response_too_large" {
		t.Errorf("title: got %q, want response_too_large", p.Title)
	}
}
