...
func TestSetup_EmptyWorkspace(t *testing.T) {
	ctx := context.Background()
	err := Setup(ctx, nil, "", "dbuser", 5*time.Second, 5*time.Second)
	if err == nil {
		t.Fatal("Setup with empty workspace_id: expected error, got nil")
	}
	if !strings.Contains(err.Error(), "empty workspace_id") {
		t.Errorf("error message: got %q, want it to contain %q", err.Error(), "empty workspace_id")
	}
}

func TestSetup_EmptyRole(t *testing.T) {
	ctx := context.Background()
	err := Setup(ctx, nil, "ws-id", "", 5*time.Second, 5*time.Second)
	if err == nil {
		t.Fatal("Setup with empty pg_role: expected error, got nil")
	}
	if !strings.Contains(err.Error(), "empty pg_role") {
		t.Errorf("error message: got %q, want it to contain %q", err.Error(), "empty pg_role")
	}
}
