...
# AC3.5 — Idempotency-Key replay returns the cached response with the replay header
...
hdr_file="$(mktemp)"
b2="$(curl -fsS -D "${hdr_file}" "${HDR[@]}" -H "Idempotency-Key: ${key}" -X POST "${URL}/v1/sql/execute" -d '{"sql":"SELECT 1","params":[]}')"
[[ "${b1}" == "${b2}" ]] && grep -qi 'X-Idempotent-Replayed: true' "${hdr_file}" \
  && pass "3.5" "idempotency replay returns identical body with X-Idempotent-Replayed" \
  || fail "3.5" "replay body differs or X-Idempotent-Replayed header missing"
rm -f "${hdr_file}"
...
if command -v agentic-coop-db >/dev/null 2>&1; then
  ...
  info "skipping AC6.3 — agentic-coop-db CLI not on PATH"
fi
