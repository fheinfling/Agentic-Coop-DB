...
attack surface is enormous. AI Coop DB lets you keep all of that internal
while still letting any HTTPS client send SQL.

### Why not Supabase / Hasura / PostgREST?

Those are great projects. AI Coop DB is intentionally smaller. It does not
...
Yes — AI Coop DB does not care. The endpoint is `{sql, params}`, so any layer
...
embedded in the server (`AICOOPDB_MIGRATE_ON_START=true` by default) and is
also shipped as a standalone binary `ai-coop-db-migrate`. For tenant tables,
...
be queued in the local SQLite retry queue (`aicoopdb.queue.Queue`) and
...
runs as `aicoopdb_owner`, which is allowed to install extensions. Make
